...
function Header(props) {
  // Recebo via props o titulo e a função a ser
  // chamada quando o usuário clicar em pesquisar
  const { title, search } = props;

  // crio um state para manter a pesquisa por nome
  const [nome, setNome] = React.useState("");

  function pesquisar() {
    // caso tenha preenchido corretamente a pesquisa
    // envio para o componente pai a chamada e limpo
    // o estado
    if (nome) {
      search(nome);
      setNome("");
  ...
  return (
    <React.Fragment>
      <AppBar position="static">
        <Toolbar
          sx={{ borderBottom: 1, borderColor: "divider", bgcolor: "blak" }}
        >
          <Link to="/">
